...
def send_menu(chat_id):
    ...
    reply_markup = {"inline_keyboard": [[btn.to_dict()] for row in keyboard for btn in row]}
    requests.post(
        TELEGRAM_API_URL,
        json={"chat_id": chat_id, "text": "Please choose an option:", "reply_markup": reply_markup},
    )

def handle_mtf_download(chat_id):
...
def fetch_groww_mtf_data():
    ...
    while True:
        try:
            ...
            if not data: break
            for s in data:
                if s.get("marketCap", 0) == 0: continue
                all_data.append({
...
async def send_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Case 1: called via /start or regular message
    if update.message:
        await update.message.reply_text(
            f"Hi {user.full_name}!\nChoose an option:", reply_markup=reply_markup
        )
    # Case 2: called from callback query (e.g., button click)
    elif update.callback_query:
        try:
            await update.callback_query.answer()  # Acknowledge the click
        except:
            pass
...
async def button_click(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if choice == "swing_trade":
        ...
        await query.message.reply_photo(photo=f"https://picsum.photos/600/400?random={random.randint(1,1000)}", reply_markup=back_markup)
